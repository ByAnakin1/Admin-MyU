...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/arnes'); // Ruta para obtener los productos de Arnes
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/arnes/${id}`); // Ruta para obtener el producto específico
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/arnes/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/arnes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/arnes/${id}`, { 
                method: 'DELETE'
            });
