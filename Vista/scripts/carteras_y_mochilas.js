...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/carteras_y_mochilas'); // Cambiado a la ruta correcta de carteras y mochilas
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/carteras_y_mochilas/${id}`); // Cambiado a la ruta correcta de carteras y mochilas
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/carteras_y_mochilas/${editingProductId}`, { // Cambiado a la ruta correcta de carteras y mochilas
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/carteras_y_mochilas', { // Cambiado a la ruta correcta de carteras y mochilas
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
