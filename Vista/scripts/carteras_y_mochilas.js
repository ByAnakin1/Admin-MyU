...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/carteras_y_mochilas'); // Ruta para carteras y mochilas
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/carteras_y_mochilas/${id}`); // Ruta para carteras y mochilas
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        const url = editingProductId 
            ? `http://localhost:3000/api/carteras_y_mochilas/${editingProductId}` 
            : 'http://localhost:3000/api/carteras_y_mochilas';
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/carteras_y_mochilas/${id}`, { method: 'DELETE' });
