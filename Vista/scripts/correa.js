...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/correas'); // Ruta para obtener los productos de Correa
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/correas/${id}`); // Ruta para obtener el producto específico
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/correas/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/correas', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/correas/${id}`, { 
                method: 'DELETE'
            });
