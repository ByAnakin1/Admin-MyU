...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/ganchos'); // Ruta para obtener los productos de Ganchos
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/ganchos/${id}`); // Ruta para obtener el producto específico
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/ganchos/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/ganchos', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/ganchos/${id}`, { 
                method: 'DELETE'
            });
