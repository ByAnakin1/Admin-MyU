...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/gargantillas'); // Ruta para obtener gargantillas
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/gargantillas/${id}`); // Ruta para obtener un producto
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/gargantillas/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/gargantillas', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/gargantillas/${id}`, { 
                method: 'DELETE'
            });
