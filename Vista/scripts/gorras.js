...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/gorras'); // Ruta de gorras
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/gorras/${id}`); // Ruta de gorras
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/gorras/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/gorras', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/gorras/${id}`, { 
                method: 'DELETE'
            });
