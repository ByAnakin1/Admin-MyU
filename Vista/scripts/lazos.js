...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/lazos'); // Ruta de lazos
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/lazos/${id}`); // Ruta de lazos
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/lazos/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/lazos', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/lazos/${id}`, { 
                method: 'DELETE'
            });
