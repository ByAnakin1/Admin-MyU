...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/lentes'); // Ruta correcta de lentes
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/lentes/${id}`); // Ruta correcta de lentes
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/lentes/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/lentes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/lentes/${id}`, { 
                method: 'DELETE'
            });
