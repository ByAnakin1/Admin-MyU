...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/medias_y_pantys'); // Ruta para medias y pantys
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/medias_y_pantys/${id}`); // Ruta para medias y pantys
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        const url = editingProductId 
            ? `http://localhost:3000/api/medias_y_pantys/${editingProductId}` 
            : 'http://localhost:3000/api/medias_y_pantys';
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/medias_y_pantys/${id}`, { method: 'DELETE' });
