...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/medias_y_pantys'); // Cambiado a la ruta correcta de medias y pantys
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/medias_y_pantys/${id}`); // Cambiado a la ruta correcta de medias y pantys
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/medias_y_pantys/${editingProductId}`, { // Cambiado a la ruta correcta de medias y pantys
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/medias_y_pantys', { // Cambiado a la ruta correcta de medias y pantys
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
