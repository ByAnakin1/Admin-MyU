...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/pines'); // Ruta correcta de pines
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/pines/${id}`); // Ruta correcta de pines
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/pines/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/pines', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/pines/${id}`, { 
                method: 'DELETE'
            });
