...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/poleras_y_casacas'); // Cambiado a la ruta correcta de poleras y casacas
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/poleras_y_casacas/${id}`); // Cambiado a la ruta correcta de poleras y casacas
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/poleras_y_casacas/${editingProductId}`, { // Cambiado a la ruta correcta de poleras y casacas
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/poleras_y_casacas', { // Cambiado a la ruta correcta de poleras y casacas
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
