...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/poleras_y_casacas'); // Ruta correcta para poleras y casacas
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/poleras_y_casacas/${id}`); // Ruta correcta para poleras y casacas
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/poleras_y_casacas/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/poleras_y_casacas', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/poleras_y_casacas/${id}`, { 
                method: 'DELETE'
            });
