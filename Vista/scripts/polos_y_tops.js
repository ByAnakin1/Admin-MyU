...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/polos_y_tops'); // Cambiado a la ruta correcta de polos y tops
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/polos_y_tops/${id}`); // Cambiado a la ruta correcta de polos y tops
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/polos_y_tops/${editingProductId}`, {
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/polos_y_tops', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/polos_y_tops/${id}`, {
                method: 'DELETE'
            });
