...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/vestidos'); // Ruta correcta de vestidos
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/vestidos/${id}`); // Ruta correcta de vestidos
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/vestidos/${editingProductId}`, { 
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/vestidos', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
...
async function deleteProduct(id) {
    if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
        try {
            const res = await fetch(`http://localhost:3000/api/vestidos/${id}`, { 
                method: 'DELETE'
            });
