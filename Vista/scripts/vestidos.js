...
async function fetchProducts() {
    try {
        const res = await fetch('http://localhost:3000/api/vestidos'); // Cambiado a la ruta correcta de vestidos
...
async function editProduct(id) {
    try {
        const res = await fetch(`http://localhost:3000/api/vestidos/${id}`); // Cambiado a la ruta correcta de vestidos
...
productForm.addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (editingProductId) {
            ...
            res = await fetch(`http://localhost:3000/api/vestidos/${editingProductId}`, { // Cambiado a la ruta correcta de vestidos
                method: 'PUT',
            ...
            res = await fetch('http://localhost:3000/api/vestidos', { // Cambiado a la ruta correcta de vestidos
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(productData)
            });
